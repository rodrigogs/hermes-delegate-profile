...
class TestLintWarnings:
    ...
    def test_a_tier_member_missing_from_the_fallback_chain_warns(self):
        """The hand-maintained union has drifted, and only lint can say so.

        ``blocklist.fallback_chain`` is documented as the union of every tier
        member and has to be regenerated by hand when ``tiers`` changes. Nothing
        checked it, so the drift was invisible until a ban landed on the missing
        model — and then it cost the escape hatch.
        """
        config = _cfg({"T1": {
            "model": "m1", "provider": "zai",
            "fallback": [{"model": "f1", "provider": "deepseek"}],
        }})
        config["blocklist"] = {"fallback_chain": ["m1"]}
        assert (
            "blocklist.fallback_chain does not list tier member 'f1' "
            "— it cannot be substituted for, and cannot be escaped to"
        ) in lint_warnings(config)

    def test_a_complete_fallback_chain_does_not_warn(self):
        config = _cfg({"T1": {
            "model": "m1", "provider": "zai",
            "fallback": [{"model": "f1", "provider": "deepseek"}],
        }})
        # Every tier member, including the ones _cfg supplies for T2-T4.
        config["blocklist"] = {"fallback_chain": ["m1", "f1", "m2", "m3", "m4"]}
        assert not any(
            "fallback_chain does not list" in w for w in lint_warnings(config)
        )

    def test_one_finding_per_missing_model_however_many_tiers_name_it(self):
        """Sorted and deduplicated, so the report is stable across processes."""
        config = _cfg({
            "T1": {"model": "shared", "provider": "zai"},
            "T2": {"model": "shared", "provider": "zai",
                   "fallback": [{"model": "other", "provider": "deepseek"}]},
        })
        # m3/m4 are _cfg's untouched T3/T4 primaries and are missing too, so this
        # also pins the ORDER: sorted, not tier order, and 'shared' appears once
        # although two tiers name it.
        config["blocklist"] = {"fallback_chain": []}
        found = [w for w in lint_warnings(config) if "does not list" in w]
        assert [w.split("'")[1] for w in found] == [
            "m3", "m4", "other", "shared",
        ]

    def test_a_malformed_blocklist_block_is_lints_question_not_ours(self):
        """No list to compare against means no drift to report — and no raise."""
        for blocklist in ("nope", None, {}, {"fallback_chain": "nope"},
                          {"fallback_chain": None}):
            config = _cfg({"T1": {"model": "m1", "provider": "zai"}})
            config["blocklist"] = blocklist
            assert not any(
                "fallback_chain does not list" in w for w in lint_warnings(config)
            )

    def test_a_non_mapping_tier_is_skipped_rather_than_raising(self):
        """A half-edited tier is lint()'s shape error, not a drift finding.

        Same reading the tier loop above uses. Reached only with a WELL-FORMED
        blocklist, since a malformed one returns before the tiers are walked.
        """
        config = _cfg({"T1": {"model": "m1", "provider": "zai"}})
        config["tiers"]["T2"] = "still-a-scalar"
        config["blocklist"] = {"fallback_chain": ["m1", "m3", "m4"]}
        assert not any(
            "fallback_chain does not list" in w for w in lint_warnings(config)
        )

    def test_a_non_string_chain_entry_is_ignored_rather_than_matched(self):
        """A numeric row cannot list a model, so the member is still missing."""
        config = _cfg({"T1": {"model": "m1", "provider": "zai"}})
        config["blocklist"] = {"fallback_chain": [7, None, "m1"]}
        assert not any(
            "does not list tier member 'm1'" in w for w in lint_warnings(config)
        )
        config["blocklist"] = {"fallback_chain": [7, None]}
        assert any(
            "does not list tier member 'm1'" in w for w in lint_warnings(config)
        )

    def test_the_shipped_policy_carries_no_fallback_chain_drift(self):
        """The invariant the warning exists to protect, asserted on what ships.

        ``tests/test_shipped_policy_names_real_rails.py`` pins the chain as the
        tier union from the other direction; this pins that the LINT agrees, so the
        ...
        shipped = yaml.safe_load(
            (pathlib.Path(__file__).resolve().parents[2] / "router.example.yaml")
            .read_text(encoding="utf-8")
        )
        assert [
            w for w in lint_warnings(shipped) if "does not list" in w
        ] == []
