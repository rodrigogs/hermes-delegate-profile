...
test('the summary facts exist nowhere else on the screen', () => {
  ...
  // Both labels come from the map now: 'classifier' was the last English one on the
  // screen, and §4.7's rule is that a phrase the surface says lives in exactly one place.
...
test('the missing-group row shows the §3.4(a) inline destination select, on the row itself', () => {
  // The control was born only under a global editing mode, with the reasoning that a
  // fix needs a save path. Writing is armed per OBJECT now, and this control's own
  // action opens the rule's editor — so the save path is what pressing it creates, and
  // withholding the remedy until something else was armed left the one row that is
  // broken as the one row with nothing to press.
  const { api, dom } = loadConsole();
  missingGroupState(api);
  api.renderSheet();
  ...
  // It does not come and go with anything global, because nothing global is armed:
  // the row that is broken always offers the one move that fixes it.
  api.setMode('reading');
  api.renderSheet();
  assert.equal(findAll(dom.get('sheet'), 'step-dest-fix').length, 1,
    'the remedy belongs to the row, not to a mode');
  api.setMode('editing');
  api.renderSheet();
  assert.equal(findAll(dom.get('sheet'), 'step-dest-fix').length, 1);
});
...
test('only a manual ban offers removal, and it is on the row that owns it', () => {
  // §3.3's control used to be drawn only under `state.mode === 'editing'`, which was
  // reachable through a global toggle this redesign deleted — so on a screen with no
  // toggle the control would never have been drawn again. It is now what every other
  // action on this surface is: a labelled button on the row that owns the object, in the
  // action slot, always visible. What stops the write is unchanged and elsewhere —
  // writable() needs the session credential and the server needs the token and a
  // matching base_hash.
  const { api, dom } = loadConsole();
  api.state.policy = {};
  api.state.blocklist = {
  ...
  assert.equal(buttons.length, 1, 'the manual ban row carries its control');
  assert.equal(buttons[0].textContent, 'Remover o bloqueio');
  assert.match(buttons[0].title, /glm-5\.3/, 'and the control names which ban it lifts');
  ...
  // The control does not come and go with anything global, because nothing global is
  // armed any more.
  api.setMode('editing');
  assert.equal(findAll(dom.get('bans'), 'btn').length, 1);
  api.setMode('reading');
  assert.equal(findAll(dom.get('bans'), 'btn').length, 1,
    'and it is still there, because the row is what owns it');
});
...
test('removing a ban plans the whole list without it, and nothing else', async () => {
  ...
  // renderHealth owns this block; nothing global has to be armed for its control to
  // exist, because the control belongs to the row.
  api.renderHealth();
...
test('ONE WRITE VOCABULARY: the same three words on every surface that writes', () => {
  ...
  assert.match(script, /\['jsonApply', WRITE\.save\], \['jsonPreview', WRITE\.plan\], \['jsonRevert', WRITE\.revert\]/,
    'one pass stamps the file editor\'s three buttons from the map');
  assert.match(script, /\['refresh', WRITE\.refresh\]/, 'and the reload button with them');
...
test('the screen speaks one language', () => {
  ...
  // The compaction block left the screen entirely (§6.3), so its type-to-confirm phrase
  // is not here to be checked — and the English strings this test guards are gone with it.
  assert.ok(!markup.includes('Type COMPACT'), 'and its English form did not survive either');
});
