...
| `plugin.yaml` | 49 | — | Hermes manifest: `name: hermes-smart-router`, v0.4.0, `provides_tools: [delegate_profile]`, `provides_hooks: [post_tool_call, pre_kanban_dispatch]`. **Was `name: delegate-profile` until 2026-09-04, and that is why nothing was ever routed**: for a flat user plugin `key == name` (`hermes_cli/plugins.py:_parse_manifest`), and that key is what `plugins.enabled` matches and what the LLM trust gate reads `plugins.entries.<plugin_id>.llm` from — both installs enabled `hermes-smart-router`, so `register()` never ran. The tool name stays `delegate_profile` (§23.2). |
